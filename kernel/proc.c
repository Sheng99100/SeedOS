...
void exit(int status)
{
  ...
  // 当前进程的 p->state 已经被设为 ZOMBIE，不会再被调度返回.
  sched();
  panic("zombie exit");
}
...
// 退出进程, 涉及到释放资源和用某种方式通知父进程
// 怎么 "通知" 父进程自己已经退出 ?
// 上面这个模糊的问句描述本身就以 "通知" 的需求场景作为基础
// 父进程一般是等待子进程退出的
// 所以不一定要把 "通知" 和资源释都放在一次性的过程里去完成
//
// 进程退出, 以 p->state == ZOMBIE 作为标志.
// 退出进程自己设置 p->state == ZOMBIE. 释放资源由父进程执行
// 父进程的 wait() 等待任意 [一个] 子进程退出, 它的 n 个子进程退出时调用 exit()
//   1. 如果父进程 wait() 时已经有子进程退出了, 那就释放子进程资源, 然后直接从 wait 返回
//   2. 如果父进程 wait() 时候还没有子进程退出, sleep
//   3. 如果父进程没有任何子进程, 退出 wait()
//
// wait 要释放 ZOMBIE 子进程 (等价的 exit 要等待被 wait 释放)
// 可以把这个需求抽象为 "配对"
// 只要一个 wait() 和一个 exit() "配对", 就能消除一对 wait() 和 exit()
// 而不管 wait() 和 exit() 的先后顺序如何
// 于是 wait 和 exit 的调用形成了一个列表
// 另外, 因为一个进程不能在 wait 还没退出的情况下又调用 wait.
// 所以对于一个进程的 wait, exit 列表中, 只能同时存在一个 wait
// 另外, wait 内访问所有进程结构, 且不是全读的操作. 所有即便是不同进程, 也不应该并发的执行 wait.
// 所以 wait 没有每个 cpu 各一个的放在 cpu 结构体里
//
// 上述 wait, exit 列表的抽象, 隐含了一个不说自明的大前提: wait, exit 是原子, 分离的
// 要么 wait 要么完全在 exit 前, 要么完全在其后.
// 否则, exit 的 p->state=ZOMBIE 可能在 wait 检查到exit对应的进程后才被设置.
// 就无法完成 "配对" 的目的
// 如果父子进程调用过 wait, exit 却不能 "配对" 消除.
// 导致 wait 一直等 exit, 或 exit 的进程资源一直等着被释放. 这就是 "丢失" 现象了.
// (sleep and wakeup) 也是类似的 "配对". 只是在sleep and wakeup 的目的基础上, wakeup 可以 "自消除"
//
// 可以把 wait 和 exit 的过程转换到下面这个抽象当中
//
// 可以把父进程想象成树的节点, 子进程调用 exit(), 才再增加一个节点到作为它的子节点
// 可以把每一次父进程调用 wait(), 想象成在父节点上画上记号 "wait".
// 最多在父节点上有一个记号 "wait".
// 记号 "wait" 和子节点进行配对.
// 1. 配对后子节点和父节点的 "wait" 记号消失,
// 2. 如果没有子节点, 但是有[子节点出现的可能], "wait" 记号会一直存在. 直到出现一个子节点 (被 exit 加上的)
// 3. 如果没有[子节点出现的可能], 即改节点代表的进程没有任何子进程
//    也就不可能有子进程调用 exit 去添加 "子节点".
//    则 "wait" 记号直接消失. 而不需要 "配对"
  ...
  for (;;)
  {
    ...
    for (pp = proc; pp < &proc[NPROC]; pp++)
    {
      if (pp->parent == p)
      {
        ...
        // wait_lock 确保 wait() 和 exit() 之间的原子性
        // 不能确保这段指令和 swtch 之间的原子性
        // 所以里面再用 p->lock
        acquire(&pp->lock);
        havekids = 1;
        if (pp->state == ZOMBIE)
        {
          // Found one.
          pid = pp->pid;
          // 如果caller用参数指定了要复制到的用户地址, 那么
          // 复制子进程的 Exit status 到用户指定的用户空间地址
          // 如果复制失败, 释放锁后返回 -1 表示出错
    ...
    // 如果该进程没有任何子进程, 那该进程的 wait() 就不需要等一个 exit() 配对了
    // 可以直接退出 wait()
    ...
    // 如果该进程至少有一个子进程, 且一个都还没有退出, 被设置为 ZOMBIE 状态, 的情况. 就 sleep 该进程
    // 等待某个子进程调用 exit() 来消除, 唤醒这个进程
    // 现在 wait() "结束", 可以让释放锁让子进程执行 exit() 了
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    // 唤醒等待 IO 操作而 sleep 的线程的 wakeup() 只会在中断 handler 中执行
...
// 即保证 “wakeup 唤醒 sleep 的线程” (描述还是模糊的) 这个时序关系
...
// 也就是 sleep -> wait
// 如果在修改状态为 SLEEPING 前释放锁（在 sleep 内）
// 意味着允许 sleep 线程和 wakeup 线程的指令穿插
// 而 wakeup 只会唤醒 SLEEPING 的线程, 但当前线程还没有改成 SLEEPING
// 意味着 sleep 的线程可能无法再被调度
// 这就是所谓的 "wakeup丢失"
...
// 除了被 sleep 线程所在的 CPU 调用 sched 后，切换到的 scheduler 调度
// 可以被任意 [一个] (scheduler之间互斥) CPU 的 scheduler 调度
...
// 这个并发情况和 yield 在设置 p->state=RUNNABLE 前获得进程锁是一样的
// 所以 ---- wakeup 应该在 sleep 彻底切换上下文后使用；具体是在切换到 sheduler 线程后调用
//
// 结合上述的几个要求
// a. 需要释放条件锁
// b. 两个时序关系要求
...
//
// 0. p->state 不该并发写
// 1. p->state=RUNNABLE 不能在设置 p->state=SLEEPING 前 (如果 sleep 在 wakeup 前)
// 2. swtch 中唤醒该条件对应的进程的部分, 不能在和在该条件等待的进程的 sleep->sched->swtch 并发执行
// a. ----- 改变条件的代码可以和该条件的 sleep 并发执行. 不会破坏 b.
// b. ----- 虽然 wakeup 设置 RUNNABLE 本身可以在设置 SLEEPING 后执行
//          但是会引发的结果是, 被 swtch, 导致相同线程的并发
//          所以 wakeup 设置对应等待进程为 RUNNABLE 的部分, 必须在彻底切换后执行
//          所以干脆锁大范围:
//          该条件的 sleep 到 scheduler 的跨线程的整个过程, 和
//          wakeup 内唤醒在该条件等待的进程的代码不能并发执行 (满足 b 一定 1, 2)
// 最小改动当前具备前提、代码的做法是在 sleep 和 wakeup 用 p->lock
// 因为 sleep 和 wakeup 的调用时序是要跨越线程的
// 在 sched() -> swtch() 前获得锁. "前" 到设置 p->state=SLEEPING 前就能同时解决问题 0 1 2
// 在 sched() -> swtch() 后，进了 scheduler() 就可以释放
// 所以 p->lock 是符合的
// 在 sleep() 一开始就获得 p->lock，进入 scheduler() 释放, 能同时满足 0, 1, 2 三个条件
// 即满足 b  就能同时满足 0, 1, 2 三个条件
//
// 持有条件锁时才能修改条件. 因为 wakeup 在条件修改后调用, 条件不改, wakeup 就不会被执行
// 所以持有条件锁的区域, 也意味着条件不会被其他线程修改条件, 也就不会和 wakeup 中唤醒在该条件睡眠的进程的代码并发
// 可以得到
// c. wakeup 中唤醒在条件 con 睡眠的进程的部分要执行, 就要获得三个条件:
//    0. 该进程的 SLEEPING 的条件是 con
//    1. 该进程在 SLEEPING 状态
//    2. 该进程的进程锁
//    3. 条件 con 的条件锁
// d. 获得 p->lock 之后就可以释放条件锁
//    如果在之后释放
//    因为根据 c, sleep() 获得在该条件等待的进程(即正在执行sleep()的进程)的 p->lock 后
//    即便条件修改了, 但是 wakeup 中唤醒在该条件等待的这个进程的部分, 也不能获得它的 p->lock
//    并和 sleep 并发
//    所以还是满足上述的条件 b.
//    如果在之前释放
//    因为还没被改成 SLEEPING, 会导致 wakeup 丢失
//
// 更好的方法是(同时是优化)
// 在切换前再检查一次条件是否满足, 如果已经满足就不要切换上下文
// 设置回 RUNNING [后], 再释放 p->lock, 并重新获得条件锁, 
// 然后从 sleep 返回, 再次进循环检查条件 (检查条件被满足到重新获得锁期可能又不满足)
// 这个方法是, 考虑到最终目的是等待条件满足, 即使 wakeup 丢失, 但是还可以直接检查条件
// 条件满足也就可以尝试获得条件锁, 并返回到循环内重新检查了
void sleep(void *chan, struct spinlock *lk)
{
  ...
  // 假设没有锁, 最原始的执行状态
  // 如果在修改状态为 SLEEPING 前释放锁
  // 虽然顺序应该是 [sleep, wakeup]
  // 但是因为 wakeup 只会唤醒正在 SLEEPING 的线程, 但当前线程还没有改成 SLEEPING
  // 导致 "wakeup 丢失"
  // Go to sleep.
  p->chan = chan;
  p->state = SLEEPING;
  ...
  // 假设没有锁, 最原始的执行状态
  // 设置 SLEEPING 和 chan 后
  // 其他线程的 wakeup 就可以设置 p->state=RUNNABLE. 于是可能被调度
  // 导致这个线程之前执行过的过程又被执行一遍, 且并发修改 sp
  sched();
